  ...
  <script src="assets/js/bootstrap.min.js"></script>
    ...
    <body >
        <?php while($row = $database->fetch_array($result)){ 
             $comm = $database->query_db("SELECT * FROM company WHERE com_id='".$row['posted_by']."'");
             $comm = $database->query_db("SELECT * FROM student WHERE student_id='".$row['posted_by']."'");
                
            $rows = $database->fetch_array($comm);
        ?>
            <p class="" style=""><span class="">FROM: <?php echo $rows['student_name']." ".$rows['company_name']; ?><br></span><?php echo $row['complain']; ?></p>
        <?php } ?>
